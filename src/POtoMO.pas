...
USES
  SysUtils, Math, msetypes, StrUtils, Classes, StreamIO;
...
FUNCTION MObuilder.HashSpace (Entries: integer): integer;
 VAR
   i: integer;
 BEGIN
   Result:= (4* Entries) DIV 3;
   IF NOT odd (Result) THEN Inc (Result);
   REPEAT
     i:= {round}floor (sqrt (Result));
     IF NOT odd (i) THEN Dec (i);
     WHILE (i > 1) AND ((Result MOD i) <> 0) DO Dec (i, 2);
     IF i > 1 THEN Inc (Result, 2);
   UNTIL i <= 1;
 END;
