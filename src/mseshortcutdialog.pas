...
type
 tmseshortcutdialogfo = class(tmseform)
   ...
   timer: ttimer;
   alternate_edit: tstringedit;
   shortcut_edit: tstringedit;
...
implementation
uses
 mseshortcutdialog_mfm,msekeyboard,msedatalist,msearrayutils,msestockobjects, captionideu;
...
begin
 ...
 try
  ...
  if result = mr_ok then begin
   ...
   fo1.sc.itemlist.expandall;
   int2:= ord(high(ss1)) + 2;
   for int1:= int2 to fo1.shortcut_grid.rowhigh do begin
...
begin
 ...
 alternate_edit.value:= sc1di.value;
 shortcut_edit.value:= scdi.value;
end;

procedure tmseshortcutdialogfo.keytimeout(const sender: TObject);
begin
 if shortcut_edit.focused then begin
  shortcut_edit.editor.selectall;
 end
 else begin
  if alternate_edit.focused then begin
   alternate_edit.editor.selectall;
  end;
 end;
end;
...
begin
 case info.eventkind of
  cek_focusedcellchanged: begin
   ...
   shortcut_edit.enabled:= bo1;
   alternate_edit.enabled:= bo1;
   defaultbu.enabled:= bo1;
   // if info.cell.row >= 0 then
   // if not tshortcutitem(sc.item).fisgroup then shortcut_edit.setfocus;
  end;
...
begin
 ...
 sc.itemlist.refreshitemvalues(shortcut_grid.row,1);
 checkconflict;
end;
...
begin
 ...
 with shortcut_grid.fixrows[-1] do begin
...
begin
 // aligny(wam_center,[shortcut_edit,defaultbu,okbu,cancelbu]);
end;
