...
{$endif}
  ...
  protected
   ...
   procedure dotimerupdown(const sender: tobject);
...
begin
 ...
 fmouseeventcol:= -1;
 fincupdown := 0;
 ftimerupdown := ttimer.create(TComponent(self)) ;
 ftimerupdown.interval := 20000;
 ftimerupdown.options := [to_leak];
 ftimerupdown.ontimer := @dotimerupdown;
 ftimerupdown.tag := 0 ;
    ...
    case info.key of
     key_up: begin
      if shiftstate = [ss_ctrl] then begin
       ...
       begin  
       ftimerupdown.tag := 1;    
       if ftimerupdown.enabled then else
       ftimerupdown.enabled := true;
       end;
